...
3. If the custom domain was switched, point Shopify callbacks and any optional desktop eBay callbacks back to the previous healthy Render URL.
...
2. Redeploy.
3. Confirm `/readyz`.
4. If optional desktop eBay OAuth is enabled, test eBay connection state for a known development shop.
...
- Optional desktop eBay callback passes in sandbox/review mode if that workflow is enabled.
